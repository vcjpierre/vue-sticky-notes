    ...
    <section>
        <div class="col-md-4 col-sm-6" v-for="(oneNote, i) in allNotes" :key="oneNote.i">
            <div v-if="oneNote.text.includes(searchKey)" class="single-note" v-bind:class="[oneNote.color]">
                ...
                <input v-if="edit && editId == i" type="text" autofocus="true" placeholder="Escribe un título..." v-model="oneNote.title">
                <h2 :class="{isDone: oneNote.completed}" v-else>{{oneNote.title}}</h2>
                <small>Última actualización: {{oneNote.date}}</small>
                <hr>
                <textarea v-if="edit && editId == i" placeholder="Escribe una descripción..." v-model="oneNote.text"></textarea>
    ...
    module.exports = {
        ...
        methods: {
            ...
            deleteNote: function(id) {
                this.confirm = confirm("¿Deseas eliminar esta nota?");
                if (this.confirm) {
